...
def train_model(mode, dname, dendsom, x_train, y_train, stop_on_task):
    if mode == 'classif':
        dendsom.train_classifier(x_train, y_train,
                                 a0=0.95,
                                 lmbd=10e2,
                                 sigma0=None)
    elif mode == 'incr_task':
        a_crit = 5e-3 if dname == 'mnist' else 5e-5
        dendsom.train_incr_task(x_train, y_train,
                                a0=0.95,
                                lmbd=10e2,
                                sigma0=None,
                                a_crit=a_crit,
                                r_exp=2,
                                stop_on_task=stop_on_task)
    elif mode == 'incr_class':
        a_crit = 5e-3 if dname == 'mnist' else 5e-5
        dendsom.train_incr_class(x_train, y_train,
                                 a0=0.95,
                                 lmbd=10e2,
                                 sigma0=None,
                                 a_crit=a_crit,
        ...
        a_crit = 5e-3 if dname == 'mnist' else 5e-5
        dendsom.train_incr_dom(x_train, y_train,
                               a0=0.95,
                               lmbd=10e2,
                               sigma0=None,
                               a_crit=a_crit,
            ...
            for metric in metrics_used:
                ...
                for stop in stop_on_task:
                    ...
                    for trial in range(n_trials):
                        ...
                        dendsom = init_model(dataset,
                                             arch,
                                             metric,
                                             X_train[0].shape)
                        shuffle = cp.random.permutation(X_train.shape[0]).tolist()
                        dendsom = train_model(mode, dataset, dendsom,
                                              X_train[shuffle],
                                              y_train[shuffle],
                                              stop_on_task=stop)
